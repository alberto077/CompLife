...
import { Plus, Check, Trash2, Zap, RotateCw, AlertCircle, Sparkles, CheckSquare } from "lucide-react";
...
export default function TasksClient({ initialSkills, initialTasks }: { initialSkills: Skill[], initialTasks: Task[] }) {
  ...
  const handleAddTask = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      toast(`Quest "${title}" created!`, "success");
  ...
  const handleToggle = (task: Task) => {
    startTransition(async () => {
      ...
      toast(`Quest completed: ${task.title}`, "xp", task.xpReward);
      setCelebrationXp(task.xpReward);
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
           ...
           <form onSubmit={handleAddTask} className="flex flex-col gap-4">
             ...
             <div className="ml-14 flex flex-wrap items-center gap-3">
               ...
               <button 
                  ...
                  className="ml-auto flex items-center gap-2 px-6 py-2 rounded-xl bg-white text-black font-semibold hover:bg-neutral-200 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      >
        <button
          ...
          className="w-full p-1 rounded-3xl bg-gradient-to-br from-purple-500/20 via-neutral-900/50 to-neutral-900/50 border border-white/5 hover:border-purple-500/30 transition-all group"
        >
          <div className="p-6 bg-neutral-950/80 rounded-[22px] backdrop-blur-xl flex items-center gap-4">
            ...
            <div className="text-left flex-1">
              <p className="text-white font-semibold">AI Goal Planner</p>
              <p className="text-sm text-neutral-400">Describe a goal and AI will break it into step-by-step quests</p>
            </div>
            <span className="text-sm font-medium text-purple-400 group-hover:text-purple-300 transition-colors">Open →</span>
          </div>
        </button>
      </motion.div>
      ...
      >
        ...
        {initialTasks.length === 0 ? (
          <div className="py-12 text-center text-neutral-500 border border-dashed border-white/10 rounded-3xl">
            ...
            <p className="mb-2">You have no active quests!</p>
            <p className="text-sm">Add one above or use the AI Goal Planner to start earning XP.</p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center gap-4">
                    <button 
                      ...
                      className={`w-6 h-6 rounded-full flex items-center justify-center border-2 transition-all disabled:opacity-50 ${
                        task.completed 
                          ? "bg-emerald-500 border-emerald-500 text-white cursor-not-allowed" 
                          : "border-neutral-500 text-transparent hover:border-emerald-500 hover:scale-110 group-hover:border-indigo-400 active:scale-90"
                      }`}
                    >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button 
                      ...
                      disabled={isPending}
                      className="w-8 h-8 rounded-full flex items-center justify-center text-neutral-500 hover:bg-red-500/20 hover:text-red-400 transition-all opacity-0 group-hover:opacity-100 disabled:opacity-0 hover:scale-110 active:scale-90"
                    >
